...
{
	/**
	 * An enum defining the various kinds of rules that can be applied to the CA's logic.
	 * @author Yaniv Tzur
	 */
	public enum Rule
	{
	 ...
	 * @param model Contains information about the automaton's internal collective state.
	 * @param view Provides a graphical view of the automaton's operation and allows the user
	 ...
	 * @param model
	 * The new model to associate this controller object with.
	 */
	 ...
	 * @param view
	 * The new view object reference to associate the controller object with.
	 */
	 ...
	 * @param limitedTimeRun Whether the automaton is to be run for a limited number of generations.
	 * @param numOfGenToRunFor If limitedTimeRun == true, the number of generations to run the
	 * 						   automaton for.
	 * @param isAlwaysPairRuleActive Whether the "Always Pair" rule should be active.
	 * @param isStationaryPairRuleActive Whether the "Stationary Pair" rule should be active.
	 * @param isLoyalPairsRuleActive Whether the "Loyal Pairs" rule should be active.
	 * @param isRememberExesRuleActive Whether the "Remember Ex's" rule should be active.
	 * @param numOfRememberedExes If isRememberExesRuleActive == true, the number of former partners'
	 * 							  relationship indices to remember.
	 */
	 ...
	 * @param isRememberExesRuleActive Whether the "Remember Ex's" rule is active. It dictates
	 * 								   that states remember former partners to avoid pairing
	 * 								   with partners worse than already encountered. 
	 * @param numOfRememberedExes The number of remembered former partners' relationship
	 * 							  indices used in order to avoid pairing with worse partners
	 * 							  than those already encountered.
	 * @return Whether the automaton can start a new run or resume an existing one with the input
	 * 		   parameters and current internal state.
	 */
	private boolean canRun(boolean isRememberExesRuleActive, int numOfRememberedExes)
	{
		if (_newGenTimer != null // Timer object exists.
			&& 
			...
			(isRememberExesRuleActive == false
			 ...
			 &&
			 numOfRememberedExes != CAView.INVALID_INPUT))) // User entered valid input.
				return true;
		return false;
	}
	 ...
	 * @param isAlwaysPairRuleActive Whether "Always Pair" rule should be active.
	 * @param isStationaryPairRuleActive Whether the "Stationary Pair" rule should be active.
	 * @param isLoyalPairsRuleActive Whether the "Loyal Pairs" rule should be active.
	 * @param isRememberExesRuleActive Whether forming a couple with former partners should
	 * 								   be avoided.
	 * @param numOfRememberedExes If forming a couple with former partners should be avoided, how
	 * 							  many former partners should be remembered.
	 */
	 ...
	 * @param currentSubset The subset of rules to fetch all subsets of.
	 * @return All subsets of the input set of rules.
	 */
	 ...
	 * @param newGenerationNumber The number of the recently calculated new generation.
	 * @param newStates The states of the recently calculated new generation.
	 * @param overallHappiness The level of overall happiness in the recently calculated new
	 * 						   generation.
	 */
	 ...
	 * @return A string containing the names of all currently active rules.
	 */
	...
	/**
	 * Calculates the states of the cells of the automaton in the next generation and returns
	 * the 2-D array of states of the 2-D automaton created.
	 * @param currentStates The states of the cells of the automaton in the current generation.
	 * @return The states of the cells of the automaton in the next generation.
	 */
	...
	/**
	 * Calculates the state of the cell denoted by the input row and column indices in the next
	 * generation and returns it.
	 * @param currStates The states of each of the cells of the automaton in the current generation.
	 * @param row The row number of the cell for which to calculate the state in the next generation
	 * @param col The Column number of the cell for which to calculate the state in the 
	 * 			  next generation
	 * @return The state of the cell given by the input row and column indices in the next generation.
	 */
	 ...
	 * @param currStates The states of the automaton's cells in the current generation.
	 * @param newStates The states of the automaton's cells in the next generation.
	 * @param row The row index of the cell representing a single man/woman.
	 * @param col The column index of the cell representing a single man/woman.
	 * @return If the attempt to create a pairing failed, returns the same state.
	 * 		   Otherwise, returns the formed couple.
	 */
	 ...
	 * @param currStates The states of the automaton's cells in the current generation.
	 * @param row The row index of the cell for which to return an array of neighbors.
	 * @param col The column index of the cell for which to return an array of neighbors.
	 * @return An array of the neighbor states of the state of the cell with the input coordinates.
	 */
	 ...
	 * @param currentState A state representing a single man/woman looking for a mate.
	 * @param neighbors The states of the neighbor cells of the cell containing the input single
	 *					state.
	 * @return All neighbor states in the input array of neighbors of the input currently inspected
	 * 		   state who might be suitable as new partners for the single man/woman represented by 
	 * 		   the state.
	 */
		...
		for (int i = 0; i < neighbors.length; i++)
			if (!State.isEmptyState(neighbors[i]) 
				...
				if ((isRuleEnabled(Rule.MEMORY_ENABLED) 
					&&
					!hadBetterPartner(currentState,neighbors[i]))
					||
					!isRuleEnabled(Rule.MEMORY_ENABLED))
					{
	 ...
	 * @param state The state for which to tell if it represents an unpaired single person.
	 * @return Whether the input state represents an unpaired single person.
	 */
	private boolean isUnpairedSingle(State state)
	{
		State.StateType stateType = state.getStateType();
		if (stateType == State.StateType.COUPLE || state.isPaired())
	 ...
	 * @param currentState One state for comparison.
	 * @param neighborState The second state for comparison.
	 * @return Whether the two states each represent a person, each with a sex opposite to the other.
	 */
	private boolean isOppositeSex(State currentState, State neighborState) 
	{
		...
		if ((currStateType == State.StateType.MAN && neighborStateType == State.StateType.WOMAN)
			||
			(currStateType == State.StateType.WOMAN && neighborStateType == State.StateType.MAN))
			return true;
		return false;
	}
	 ...
	 * @param currentState Represents a single man/woman.
	 * @param neighborState Represents either a single person of the opposite sex or a couple.
	 * @return A relationship index based on the singular relationship indices of the two input
	 * 		   potential partners.
	 */
	 ...
	 * The first partner is a single person while the second, their neighbor, may be either
	 * single or currently part of a couple.
	 * @param currIndex The index of the current state considering to create a couple with
	 * 					the currently inspected neighbor state.
	 * @param neighborIndex The neighbor's relationship index.
	 * @return A relationship index based on the singular relationship indices of the two input
	 * 		   potential partners.
	 */
	 ...
	 * enabled or not.
	 * @param rule The rule for which to check if it is enabled or not.
	 * @return Whether the input rule is enabled.
	 */
	 ...
	 * value.
	 * @param rule The rule for which to set whether it is enabled or not.
	 * @param isEnabled Whether the input rule should be enabled or not.
	 */
	 ...
	 * @param rule The rule to turn off.
	 */
	 ...
	 * @param ruleIndex The ordinal number of the desired rule.
	 * @return If a rule with the input ordinal number exists, returns the corresponding rule.
	 * 		   Otherwise, returns null.
	 */
	 ...
	 * @param currentState One state of a single man/woman.
	 * @param chosenPartner Another single man/woman state to pair the first with.
	 * @param combinedRelationshipIndex Represents the level of compatibility between the two
	 * 									persons represented by the input states.
	 * @return A new state representing a man-woman couple.
	 */
	...
	{
		State.StateType currentStateType = currentState.getStateType();
		State maleState = null;
		State femaleState = null;
		int manIndex = 0;
		int womanIndex = 0;
	 ...
	 * @param currentState Expected to represent a single man/woman only.
	 * @param chosenPartner Can represent a single person or a couple.
	 */
	private void addToMemory(State currentState, State chosenPartner, int manIndex, int womanIndex)
	{
		...
		int currentStateNewIndex = 0;
		int chosenPartnerStateNewIndex = 0;
	 ...
	 * @param currStates The states of the automaton's cells in the current generation.
	 * @param newStates The states of the automaton's cells in the next generation.
	 * @param row The index of the row of a cell in the 2-D automaton.
	 * @param col The index of the column of a cell in the 2-D automaton.
	 * @return The position of the cell denoted by the input row and column indices in the 
	 * 		   2-D automaton.
	 */
	 ...
	 * @param states The states of the automaton's cells.
	 * @return The overall level of happiness among the people represented by the states of the
	 * 		   automaton's cells.
	 */
	 ...
	 * @param currStates The states of cells in the current generation.
	 * @param newStates The states of cells in the next generation.
	 * @param row The index of the row of the cell for which to calculate new possible positions
	 * 			  for the object represented by the cell's state.
	 * @param col The index of the column of the cell for which to calculate new possible positions
	 * 			  for the object represented by the cell's state.
	 * @return All possible new positions for the object represented by the state of the cell
	 * 		   whose coordinates are given by the input row and column indices.
	 */
														...
														int row, int col)
	{
		...
		if (currStates[row][col] != null && currStates[row][col].isPaired() == false
			...
			{
				possibleNewPositions = new ArrayList<Position>();
	 ...
	 * index.
	 * @param currentState The state for which to check if it has had a better partner.
	 * @param partnerState The state for which to check if it is a match that is at least as good as
	 * 					   remembered past states. 
	 * @return Whether the state has already had a past partner (including its current partner
	 * 		   if the state is of a COUPLE type) who is a better match in terms of their relationship
	 * 		   index. 
	 */
	private boolean hadBetterPartner(State currentState, State partnerState) 
	{
		...
		int pastCombRelationshipIndex = 0;
